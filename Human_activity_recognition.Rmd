...
## Introduction
Human Activity Recognition (HAR) - has emerged as a key research area in wearable device technology in the last decade. In particular for the development of context-aware systems. There are many potential applications for HAR, These include life log systems for monitoring energy expenditure and supporting weight-loss programs, and digital assistants for weight lifting exercises. These promote healthy living and weight loss programs.
...
The goal of this project is to predict the manner in which they did the exercise. This is recorded in the variable *classe* in the training set.  This report describe how the model was build trained and cross validated, 
...
The first step of the data analysis task involves loading and opening the appropriate packages and libraries used to develop the classification models. The powerful and versatile "caret" machine learning package will be employed for this assignment, The "Rattle" package will be used to illustrate the classification tree and the "gbm" and "random Forest" packages will be used to develop the random forest  and adaptive boosting models. 
```{r}
library (lattice)
library(ggplot2)
library(caret)
...
The next step involves downloading the training and testing data sets: 
...
The testing and training data sets include 160 different variables with many variables containing NA entries or 0 entries. There are 5 different  outcome variables which are listed under the variable classe (variable-160). The variable "classe" is a factor variable which is made up of letters A, B, C, D and E.  

Prior to attempting to design the predictive models, the data sets need to be cleaned. in the first instance this involves variables which contain mostly NA entries or have near zero variance: 
...
Variables 1 to 6 are for case identification and are unlikely to contribute to the predictive ability of the machine learning model. They includes:
...
They are therefore excluded from the training and testing datasets:
...
In order to examine the relationships between the variable classe and the remaining variables multivariate regression analysis is performed so that variables which bear no correlation with the classe variable are identified and removed if necessary:
...
In order to develop the optimum classification model for performing the predictions associated with this task we are going to use the following models: 

1- A random forest classifier 

2- A Classification and regression tree

Each model is trained using a proportion (70%) of the training data and validated using a cross Validation dataset (30%) of the training dataset. The final model is then tested on testingAS to assess the generalisability of  each classifier. This may be seen as a superfluous step but it is the only means of cross validation.The training dataset is called "trainingAS-t" The cross validation dataset is called "crossVal"
...
First model which we are going to develop is a random forest classifier. The random forest classifier performs repeated internal sampling and cross validation, however in order to maintain a level playing field between the classifiers it is trained and cross validated using the data partitions used: 
...
It is possible to examine the relative importance of each variable in constructing the eventual random forest classifier. 

The following is a graphical representation of this:
...
The accuracy of prediction using the tree classifier was assessed using the cross validation dataset:
...
Finally a similar prediction i.e. the prediction of the "classe" variable was performed using the decision tree classifier:  
```{r}
...
As one would expect the random forest classifier was significantly better than the classification tree at correctly classifying the "classe" variable. This is not surprising as ensemble classifiers such as random forests and adaptive boosting algorithms are some of the most accurate classifiers in use today, both for classification and regression problems. They achieve this level of accuracy by using the collective power of imperfect "weak" classifiers (3). 

Random forests work through bagging which was described earlier whilst adaptive boosting algorithms improve the accuracy of classification through iterative improvements in the classification process. The reason why I selected random forest algorithm rather than an adaboost (adaptive boosting classifier is that the latter are prone to noise and sensitive outlying variables and therefore need to be used with caution. In this task the random forest classifier was 100% correct at classifying the 20 test samples which were part of the quiz. 
